    ...
    try:
        ...
        for t in keyfob_transactions.json()["Transactions"]:
            try:
                ...
                c.execute(
                    "INSERT INTO transactions (card_id,cardtransactionid, description, location, transactiondatetime, hop_balance_display, value, value_display, journey_id, refundrequested, refundable_value, transaction_type_description, transaction_type) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    tran,
                )
                ...
                json = [{"type": "section", "text": {"type": "mrkdwn", "text": msg}}]
        ...
        if SLACK_TOKEN and SLACK_CHANNEL:
            ...
            for json in slack_messages:
                sc.chat_postMessage(
                    channel=SLACK_CHANNEL, icon_emoji=":robot_face:", blocks=json
                )
